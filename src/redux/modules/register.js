...
export function save( values ) {
  return {
    ...
    promise: client =>
      client
        .fetchJSON('https://chaus.now.sh' + uris.apis.events, 'POST', {
          name: values.event
        })
        .then(()=>
          client
            .fetchJSON('https://chaus.now.sh' + uris.apis.events, 'GET', {
              name: values.event
            })
            .then((res)=>
              res.items[0]
        ...
        .then((event)=>{
          const promises = values.members.map((member)=>
            client
              .fetchJSON('https://chaus.now.sh' + uris.apis.attendees, 'POST', {
                event: event.id,
                user: member.id
              })
          );
